...
lib.createTxtFile = (filePath, data) => {
    let str = config.batch.txtTitle.join('\t') + '\r\n';
    str += data.map(item => {
        let arr = [];
        arr.push(config.assets.platNo);
        config.batch.keys.forEach(key => {
            arr.push(item[key])
        })
        return arr.join('\t');
    }).join('\r\n');
    return lib.writeFile(filePath, str);
}
...
// let p1 = path.join(process.cwd(), './../public/batch', 'a456.txt');
// let d1 = [
//     {
//         'orderNo': '123123',
//         'creditor': '345345',
...
//         'expiryDate': 'ertyerty',
//         'use': 'ertyert',
//         'guaranteeWay': 'dfgghdfg',
//         'guarantor': 'cvbncvbn',
//         'guarantorId': 'ghjkghjk',
//         'filename': 'vbnmvbnm'
//     },
//     {
//         'orderNo': '123123',
